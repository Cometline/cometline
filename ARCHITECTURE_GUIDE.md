...
| `GET` | `/api/v1/workspaces/files` | List previewable files in a workspace |
| `GET` / `PUT` | `/api/v1/workspaces/files/content` | Read or write previewable workspace file content |
...
| `DELETE` | `/api/v1/sessions/{id}` | Delete a session and cascade messages/tool calls; gallery media stays |
| `GET` | `/api/v1/sessions/{id}/media/{mediaId}` | Fetch assistant media while the session still exists |
| `GET` | `/api/v1/media` | List ready gallery items |
| `GET` | `/api/v1/media/{id}/content` | Fetch gallery media after the session is gone |
| `DELETE` | `/api/v1/media/{id}` | Tombstone one gallery item and delete its file |
...
| `GET` | `/api/v1/skill-drafts` | List skill drafts generated by tools or users |
| `GET` / `PUT` | `/api/v1/skill-drafts/{name}` | Read or update one skill draft |
| `POST` | `/api/v1/skill-drafts/{name}/promote` | Promote a draft into a managed skill |
| `DELETE` | `/api/v1/skill-drafts/{name}` | Reject a draft |
| `GET` | `/api/v1/mcp/servers` | Inspect configured MCP server status |
| `POST` | `/api/v1/mcp/servers/{id}/oauth-flows` | Start native-browser OAuth for a remote MCP server |
| `GET` / `POST` | `/api/v1/jobs` | List or create durable jobs |
| `PUT` / `PATCH` / `DELETE` | `/api/v1/jobs/{id}/lease` | Claim, heartbeat, or release a job lease |
...
| `assistant_image` | media identity and metadata | Persisted assistant image; chat uses the session-media URL while the session lives |
...
Electron app ready
  ...
  -> Svelte layout starts health polling, settings load, workspace/session load, retention sync, job notification polling
```
...
```text
Settings modal save
  -> renderer normalizes draft and computes persistence/restart intent
...
```text
Electron main process: cometline/electron/src/main.ts
