...
func ProviderAPIKey(c *Config, entry *ProviderEntry, method string) (string, error) {
	...
	case ProviderOpenAI, ProviderOpenAICompat, ProviderOpencodeGo:
		...
		return k, nil
	case ProviderCodex, ProviderXAI, ProviderOllama:
		return "", nil
	default:
		return "", fmt.Errorf("unknown provider %q (use %q, %q, %q, %q, %q, %q, or %q)", method, ProviderAnthropic, ProviderOpenAI, ProviderOpenAICompat, ProviderOpencodeGo, ProviderCodex, ProviderXAI, ProviderOllama)
	}
}
