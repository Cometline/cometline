...
func TestRunner_InvalidToolArgumentsStayRecoverableAndOpenCircuit(t *testing.T) {
	dir := t.TempDir()
	if err := os.WriteFile(filepath.Join(dir, "hello.txt"), []byte("world"), 0o644); err != nil {
		t.Fatalf("write hello.txt: %v", err)
	}
	...
	provider := &capturingSequentialFakeProvider{sequences: [][]cometsdk.Event{
		...
		{
			cometsdk.TextDeltaEvent{Text: "I will retry with valid JSON."},
			cometsdk.StepFinishEvent{FinishReason: cometsdk.FinishStop},
			cometsdk.DoneEvent{},
		},
	}}
	...
	events, err := runAndDrain(t, r, session.AgentTurn{ID: "s-schema", ModelID: "m"})
	if err != nil {
		t.Fatalf("Run returned error: %v", err)
	}
