...
Recent example: `memory_compaction_completed` was added this way to report manual/automatic memory compaction results.
...
- ChatGPT Codex and xAI Grok use local subscription/OAuth sessions instead of API keys (`~/.codex/auth.json` for Codex; a borrowed Grok session token for xAI)
...
The `delegate_coding_task` tool spawns an external coding harness (OpenCode, Claude Code, or Codex) through a fixed, non-interactive CLI profile owned by CometMind (`cometmind/internal/acp`). Users only pick which harness to use — the command, arguments, and permission behavior are not user-editable.
...
CometMind connects to external MCP servers and exposes their tools to the **main agent only** (not coding-harness child processes). Tools are registered as `mcp_{serverId}_{toolName}` (provider-safe; invalid characters become `_`) alongside built-in tools.

**Config:** Settings → CometMind → MCP, persisted in `cometmind.mcp` inside `~/.cometmind/cometline-settings.json`.

**OAuth (remote servers):** For OAuth-protected remote MCP servers (e.g. the [Atlassian Remote MCP server](https://www.atlassian.com/platform/remote-mcp-server) at `https://mcp.atlassian.com/v1/mcp/authv2`), users just click **Connect with OAuth** — no client ID or endpoint URLs to paste. CometMind drives the full spec-compliant flow itself:
...
This project is indexed by GitNexus as **cometline** (14905 symbols, 42978 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
